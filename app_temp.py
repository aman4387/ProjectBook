...
@app.route("/")
def index():
    return render_template("error.html", message="temp")
...
def login():
    ...
    if db.execute("SELECT username from userdata where username = :username", {"username": username}).rowcount == 0:
        return render_template("index.html", message_alert="username does not exist")
    elif password != db.execute("SELECT password from userdata where username = :username", {"username": username}).fetchone()[0]:
        return render_template("index.html", message_alert="username or password is wrong")
    else:
        """ Temporary Code """
